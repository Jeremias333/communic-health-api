...
def get_id_name_rating():
	...
	with sqlite3.connect("banco.db") as con:
		cursor = con.cursor()
		cursor.execute("SELECT indice, nome, nota_avaliacao FROM centers ORDER BY nota_avaliacao DESC;")
...
def get_id_name_address():
	...
	with sqlite3.connect("banco.db") as con:
		cursor = con.cursor()
		cursor.execute("SELECT indice, nome, localizacao FROM centers;")
